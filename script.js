...
function setupDeviceStatusListener() {
    db.ref('deviceStatus').on('value', snapshot => {
        isDeviceOnline = snapshot.val() || false;
        updateStatusUI();
        
        if (!isDeviceOnline) {
            db.ref('relayState').set(false);
            updateSessionTimer(null);
        }
    });
}
...
function initialize() {
    ...
    setInterval(() => db.ref('deviceStatus').once('value', snap => {
        isDeviceOnline = snap.val();
        updateStatusUI();
    }), CHECK_INTERVAL);
}
